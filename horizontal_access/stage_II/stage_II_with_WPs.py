...
OUT_DIR = f"./output/WithWPs_{date.date(date.now()).isoformat()}_{date.now().strftime('%Hh%M')}"
...
while not satisfied:
    ...
    arcpen = Jarc.J()
    ufppen = Jufp.J()
    print(f'length penalty={jl:.2E}, ccdist={ccdist:.2E}, csdist={csdist:.2E}, sumcs={sumcs:.2E}, summscs={summscs:.2E}, jln={jln:.2E}, cpdist={cpdist:.2E}, arcpen={arcpen:.2E}, ufp={ufppen:.2E}\n')

    if jl > 1:
        print('coils too long')
        LENGTH_WEIGHT *= 1.5
        satisfied = False

    if ccdist > 1E-4:
        print('coils too close to one another')
        CC_WEIGHT *= 1.5
        satisfied = False

    if csdist > 1E-4:
        print('coils too close to surface')
        CS_WEIGHT *= 1.5
        satisfied = False

    if sumcs > 1:
        print('coils max curvature exceeded')
        CURVATURE_WEIGHT *= 1.5
        satisfied = False

    if summscs > 1:
        print('coils mean curvature exceeded')
        MSC_WEIGHT *= 1.5
        satisfied = False
        
    if jln > 1E-3:
        print('Coils are probably linked')
        LNUM_WEIGHT *= 1.5
        satisfied = False
